...
import "react-toastify/dist/ReactToastify.css";
import {
  FaMotorcycle, FaUser, FaCalendarAlt, FaClock, FaMapMarkerAlt,
  FaPhone, FaEnvelope, FaRupeeSign, FaCheckCircle, FaTimesCircle,
  FaClock as FaPending, FaEye, FaFileAlt, FaIdCard, FaSearch,
  FaFilter, FaDownload, FaPrint, FaArrowLeft, FaInfoCircle,
  FaImage, FaFilePdf, FaFileImage, FaExpand, FaTrash, FaHistory,
  FaUndo, FaUserCircle, FaSpinner, FaPassport, FaShieldAlt,
} from "react-icons/fa";
import { FaHelmetSafety } from "react-icons/fa6";
...
const AdminBikeBookings = () => {
  ...
  const [stats, setStats] = useState({
    ...
    confirmed: 0, active: 0, completed: 0, cancelled: 0,
  });
  ...
  const fetchBookings = async () => {
    try {
      ...
      const response = await axios.get("http://localhost:5000/api/bikes/admin/bookings", {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.data.success) {
        setBookings(response.data.data);
  ...
  const getFileIcon = (mimetype) => {
    if (mimetype?.startsWith("image/")) return <FaFileImage className="text-blue-500 text-2xl" />;
    if (mimetype === "application/pdf") return <FaFilePdf className="text-red-500 text-2xl" />;
    return <FaFileAlt className="text-gray-500 text-2xl" />;
  };
  ...
  const handleApprove = async (bookingId) => {
    ...
    try {
      const token = localStorage.getItem("token") || sessionStorage.getItem("token");
      await axios.put(`http://localhost:5000/api/bikes/admin/bookings/${bookingId}/approve`, {},
        { headers: { Authorization: `Bearer ${token}` } });
      setShowConfirmModal(false);
      setShowDetailsModal(false);
      setResultType("approved");
  ...
  const handleReject = async (bookingId) => {
    ...
    try {
      const token = localStorage.getItem("token") || sessionStorage.getItem("token");
      await axios.put(`http://localhost:5000/api/bikes/admin/bookings/${bookingId}/cancel`,
        { reason: rejectionReason },
        { headers: { Authorization: `Bearer ${token}` } });
      setShowRejectModal(false);
      setRejectionReason("");
  ...
  const handleFilterChange = (status) => {
    setFilterStatus(status);
    setFilteredBookings(status === "all" ? bookings : bookings.filter((b) => b.status === status));
  };

  const handleSearch = (e) => {
    const term = e.target.value.toLowerCase();
    setSearchTerm(term);
    let filtered = filterStatus !== "all" ? bookings.filter((b) => b.status === filterStatus) : bookings;
    setFilteredBookings(filtered.filter((b) =>
      b.user?.name?.toLowerCase().includes(term) ||
      b.user?.email?.toLowerCase().includes(term) ||
      b.bike?.bikeName?.toLowerCase().includes(term) ||
      b.confirmationCode?.toLowerCase().includes(term)
  ...
  const getStatusBadge = (status) => {
    const cfg = {
      pending: { color: "bg-yellow-100 text-yellow-800", icon: FaPending, label: "Pending" },
      approved: { color: "bg-blue-100 text-blue-800", icon: FaCheckCircle, label: "Approved" },
      ...
      active: { color: "bg-purple-100 text-purple-800", icon: FaMotorcycle, label: "Active" },
      completed: { color: "bg-gray-100 text-gray-800", icon: FaCheckCircle, label: "Completed" },
  ...
  const formatCurrency = (amount) => `रु ${amount?.toLocaleString("en-NP") || 0}`;

  const getConfirmModalStyles = () => {
    if (confirmType === "approve") return { icon: <FaCheckCircle className="text-green-600 text-5xl mb-4" />, buttonClass: "bg-green-600 hover:bg-green-700", buttonText: "Yes, Approve" };
    if (confirmType === "reject") return { icon: <FaTimesCircle className="text-red-600 text-5xl mb-4" />, buttonClass: "bg-red-600 hover:bg-red-700", buttonText: "Yes, Reject" };
    return { icon: <FaCheckCircle className="text-blue-600 text-5xl mb-4" />, buttonClass: "bg-blue-600 hover:bg-blue-700", buttonText: "Yes, Proceed" };
  };
  ...
  return (
    <div className="p-8">
      ...
      {showConfirmModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative w-full max-w-md mx-4" style={{animation: "modalPop 0.35s cubic-bezier(0.34,1.56,0.64,1) both"}}>
            {/* Glow ring */}
            <div className={`absolute -inset-1 rounded-3xl blur-xl opacity-60 ${confirmType === "approve" ? "bg-gradient-to-r from-green-400 to-emerald-400" : "bg-gradient-to-r from-red-400 to-rose-400"}`} />
            <div className="relative bg-white rounded-3xl shadow-2xl overflow-hidden">
              {/* Top color bar */}
              <div className={`h-2 w-full ${confirmType === "approve" ? "bg-gradient-to-r from-green-400 to-emerald-500" : "bg-gradient-to-r from-red-400 to-rose-500"}`} />
              <div className="p-8 text-center">
                {/* Icon circle */}
                <div className={`w-24 h-24 mx-auto mb-6 rounded-full flex items-center justify-center shadow-lg ${confirmType === "approve" ? "bg-gradient-to-br from-green-400 to-emerald-500" : "bg-gradient-to-br from-red-400 to-rose-500"}`}
                  style={{animation: "iconBounce 0.5s 0.2s cubic-bezier(0.34,1.56,0.64,1) both"}}>
                  {confirmType === "approve"
                    ? <svg viewBox="0 0 24 24" className="w-12 h-12 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                    : <svg viewBox="0 0 24 24" className="w-12 h-12 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
                  }
                </div>
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={executeAction} disabled={actionLoading}
                    ...
                    {actionLoading ? <FaSpinner className="animate-spin" /> : modalStyles.buttonText}
                  </button>
                </div>
              </div>
            </div>
      ...
      {showResultAnimation && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center pointer-events-none">
          <div className="absolute inset-0 bg-black/30 backdrop-blur-sm" />
          <div className="relative flex flex-col items-center justify-center" style={{animation: "resultFadeIn 0.4s ease-out both"}}>
            {resultType === "approved" ? (
              <>
                <div className="w-40 h-40 rounded-full bg-gradient-to-br from-green-400 to-emerald-500 flex items-center justify-center shadow-2xl shadow-green-500/50 mb-6"
                  style={{animation: "resultBounce 0.6s cubic-bezier(0.34,1.56,0.64,1) both"}}>
                  <svg viewBox="0 0 24 24" className="w-20 h-20 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"
                    style={{animation: "drawCheck 0.4s 0.3s ease-out both", strokeDasharray: 30, strokeDashoffset: 30}}>
                    <polyline points="20 6 9 17 4 12"/>
                  </svg>
                </div>
                <div className="text-center text-white" style={{animation: "resultFadeIn 0.4s 0.5s ease-out both", opacity: 0}}>
                  <p className="text-4xl font-black tracking-wide drop-shadow-lg">APPROVED!</p>
                  <p className="text-green-200 mt-2 text-lg font-medium">Bike booking has been approved</p>
                </div>
                {/* Confetti dots */}
                {[...Array(12)].map((_, i) => (
                  <div key={i} className="absolute w-3 h-3 rounded-full"
                    style={{
                      background: ["#10b981","#34d399","#6ee7b7","#fbbf24","#f59e0b","#3b82f6","#60a5fa","#a78bfa","#f472b6"][i % 9],
                      animation: `confetti${i} 1s 0.3s ease-out both`,
                      top: "50%", left: "50%",
                    }} />
                ))}
              ...
              <>
                <div className="w-40 h-40 rounded-full bg-gradient-to-br from-red-400 to-rose-500 flex items-center justify-center shadow-2xl shadow-red-500/50 mb-6"
                  style={{animation: "resultBounce 0.6s cubic-bezier(0.34,1.56,0.64,1) both"}}>
                  <svg viewBox="0 0 24 24" className="w-20 h-20 text-white" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                ...
                <div className="text-center text-white" style={{animation: "resultFadeIn 0.4s 0.5s ease-out both", opacity: 0}}>
                  <p className="text-4xl font-black tracking-wide drop-shadow-lg">REJECTED</p>
                  <p className="text-red-200 mt-2 text-lg font-medium">Bike booking has been rejected</p>
                </div>
              </>
            )}
          </div>
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500 mt-2">Review and manage all bike bookings</p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 mb-8">
        {[
          { label: "Total", value: stats.total, cls: "bg-white" },
          { label: "Pending", value: stats.pending, cls: "bg-yellow-50 border-l-4 border-yellow-500", txtCls: "text-yellow-600", valCls: "text-yellow-700" },
          { label: "Approved", value: stats.approved, cls: "bg-blue-50 border-l-4 border-blue-500", txtCls: "text-blue-600", valCls: "text-blue-700" },
          { label: "Rejected", value: stats.rejected, cls: "bg-red-50 border-l-4 border-red-500", txtCls: "text-red-600", valCls: "text-red-700" },
          { label: "Confirmed", value: stats.confirmed, cls: "bg-green-50 border-l-4 border-green-500", txtCls: "text-green-600", valCls: "text-green-700" },
          { label: "Active", value: stats.active, cls: "bg-purple-50 border-l-4 border-purple-500", txtCls: "text-purple-600", valCls: "text-purple-700" },
          { label: "Completed", value: stats.completed, cls: "bg-gray-50 border-l-4 border-gray-500", txtCls: "text-gray-600", valCls: "text-gray-700" },
          { label: "Cancelled", value: stats.cancelled, cls: "bg-red-100 border-l-4 border-red-700", txtCls: "text-red-700", valCls: "text-red-800" },
        ].map((s) => (
          <div key={s.label} className={`rounded-lg shadow p-4 ${s.cls}`}>
            <p className={`text-sm ${s.txtCls || "text-gray-500"}`}>{s.label}</p>
            <p className={`text-2xl font-bold ${s.valCls || "text-gray-800"}`}>{s.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white rounded-lg shadow mb-6 p-4">
        <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
          <div className="flex gap-2 overflow-x-auto pb-2">
            {[["all","purple"],["pending","yellow"],["approved","blue"],["rejected","red"],["confirmed","green"],["cancelled","red"]].map(([s, c]) => (
          ...
          <div className="relative w-full md:w-64">
            <FaSearch className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400" />
            <input type="text" placeholder="Search by name, email, bike..." value={searchTerm} onChange={handleSearch}
              className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow overflow-hidden">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                {["Booking ID","Customer","Bike","Dates","Total","Status","Actions"].map((h) => (
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredBookings.map((booking) => (
                <tr key={booking._id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 text-sm font-medium text-gray-900">{booking.confirmationCode || booking._id.slice(-6)}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <FaMotorcycle className="text-gray-400" />
                      <span className="text-sm text-gray-900">{booking.bike?.bikeName}</span>
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="text-sm text-gray-900">{formatDate(booking.pickupDate)} - {formatDate(booking.returnDate)}</div>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex space-x-2">
                      <button onClick={() => { setSelectedBooking(booking); setShowDetailsModal(true); }}
                        className="p-2 text-purple-600 hover:bg-purple-50 rounded-lg transition" title="View Details">
                        <FaEye />
                      </button>
                      <button onClick={() => fetchDocuments(booking._id)}
                        className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition" title="View Documents">
                        <FaFileAlt />
                      </button>
                      {booking.status === "pending" && (
                        <>
                          <button onClick={() => showConfirmation(handleApprove, booking._id, "Approve Bike Booking", `Approve booking for ${booking.bike?.bikeName}?`, "approve")}
                            className="p-2 text-green-600 hover:bg-green-50 rounded-lg transition" title="Approve" disabled={actionLoading}>
                            <FaCheckCircle />
                          </button>
                          <button onClick={() => { setSelectedBooking(booking); setShowRejectModal(true); }}
                            className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition" title="Reject" disabled={actionLoading}>
                            <FaTimesCircle />
                          </button>
                        </>
                      )}
        ...
        {filteredBookings.length === 0 && (
          <div className="text-center py-12"><p className="text-gray-500">No bike bookings found</p></div>
        )}
      </div>
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 p-6">
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-3">
                  <FaInfoCircle className="text-purple-600 text-2xl" />
                  <h3 className="text-xl font-bold text-gray-800">Bike Booking Details</h3>
                </div>
                <button onClick={() => setShowDetailsModal(false)} className="text-gray-400 hover:text-gray-600"><FaTimesCircle size={24} /></button>
              </div>
            </div>
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div className="bg-gray-50 rounded-lg p-4">
                  <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2"><FaFileAlt className="text-purple-600" /> Booking Info</h4>
                  <div className="space-y-2 text-sm">
                    <p><span className="text-gray-600">ID:</span> <strong>{selectedBooking.confirmationCode || selectedBooking._id}</strong></p>
                    <p><span className="text-gray-600">Status:</span> {getStatusBadge(selectedBooking.status)}</p>
                    <p><span className="text-gray-600">Created:</span> {formatDateTime(selectedBooking.createdAt)}</p>
                    {selectedBooking.approvedAt && <p><span className="text-gray-600">Approved:</span> {formatDateTime(selectedBooking.approvedAt)}</p>}
                  </div>
                </div>
                <div className="bg-gray-50 rounded-lg p-4">
                  <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2"><FaUser className="text-purple-600" /> Customer</h4>
                  <div className="space-y-2 text-sm">
                    <p><span className="text-gray-600">Name:</span> <strong>{selectedBooking.user?.name}</strong></p>
                    <p><span className="text-gray-600">Email:</span> {selectedBooking.user?.email}</p>
                    <p><span className="text-gray-600">Phone:</span> {selectedBooking.user?.phone || "Not provided"}</p>
                  </div>
                </div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2"><FaMotorcycle className="text-purple-600" /> Bike Details</h4>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                  <div><p className="text-gray-500">Bike</p><p className="font-medium">{selectedBooking.bike?.bikeName}</p></div>
                  <div><p className="text-gray-500">Number</p><p className="font-medium">{selectedBooking.bike?.bikeNumber}</p></div>
                  <div><p className="text-gray-500">Type</p><p className="font-medium">{selectedBooking.bike?.bikeType}</p></div>
                  <div><p className="text-gray-500">Engine</p><p className="font-medium">{selectedBooking.bike?.engineCapacity}</p></div>
                </div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2"><FaCalendarAlt className="text-purple-600" /> Rental Details</h4>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  <div><p className="text-gray-500">Pickup</p><p className="font-medium">{formatDate(selectedBooking.pickupDate)} at {selectedBooking.pickupTime}</p><p className="text-gray-500">{selectedBooking.pickupLocation}</p></div>
                  <div><p className="text-gray-500">Return</p><p className="font-medium">{formatDate(selectedBooking.returnDate)} at {selectedBooking.returnTime}</p><p className="text-gray-500">{selectedBooking.dropoffLocation}</p></div>
                </div>
                <div className="mt-4 pt-4 border-t border-gray-200 text-sm space-y-1">
                  <p><span className="text-gray-600">Duration:</span> <strong>{selectedBooking.totalDays} days</strong></p>
                  <p><span className="text-gray-600">Experience:</span> <strong>{selectedBooking.riderExperience || "N/A"}</strong></p>
                  <div className="flex gap-4 mt-2">
                    {selectedBooking.extraHelmet && <span className="flex items-center gap-1 text-gray-600"><FaHelmetSafety className="text-purple-500" /> Extra Helmet</span>}
                    {selectedBooking.ridingGear && <span className="flex items-center gap-1 text-gray-600"><FaShieldAlt className="text-purple-500" /> Riding Gear</span>}
                  </div>
                </div>
              </div>
              <div className="bg-gray-50 rounded-lg p-4 mb-6">
                <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2"><FaRupeeSign className="text-purple-600" /> Pricing</h4>
                <div className="space-y-2 text-sm">
                  <div className="flex justify-between"><span className="text-gray-600">Base Price ({selectedBooking.totalDays} days)</span><span>{formatCurrency(selectedBooking.basePrice)}</span></div>
                  {selectedBooking.extraHelmet && <div className="flex justify-between"><span className="text-gray-600">Extra Helmet</span><span>{formatCurrency(100 * selectedBooking.totalDays)}</span></div>}
                  {selectedBooking.ridingGear && <div className="flex justify-between"><span className="text-gray-600">Riding Gear</span><span>{formatCurrency(200 * selectedBooking.totalDays)}</span></div>}
                  <div className="flex justify-between"><span className="text-gray-600">Service Fee</span><span>{formatCurrency(selectedBooking.serviceFee)}</span></div>
                  <div className="border-t pt-2 flex justify-between font-bold"><span>Total</span><span className="text-purple-600">{formatCurrency(selectedBooking.totalAmount)}</span></div>
                </div>
              </div>
              {selectedBooking.emergencyContact && (
                <div className="bg-gray-50 rounded-lg p-4 mb-6">
                  <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2"><FaPhone className="text-purple-600" /> Emergency Contact</h4>
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div><p className="text-gray-500">Name</p><p className="font-medium">{selectedBooking.emergencyContact.name}</p></div>
                    <div><p className="text-gray-500">Phone</p><p className="font-medium">{selectedBooking.emergencyContact.phone}</p></div>
                    {selectedBooking.emergencyContact.relationship && <div><p className="text-gray-500">Relationship</p><p className="font-medium">{selectedBooking.emergencyContact.relationship}</p></div>}
                  </div>
                </div>
              )}
              <div className="flex gap-3 justify-between">
                <button onClick={() => fetchDocuments(selectedBooking._id)}
                  className="px-5 py-2 border-2 border-blue-600 text-blue-600 rounded-lg hover:bg-blue-50 transition font-medium flex items-center gap-2">
                  <FaFileAlt /> View Documents
                </button>
                {selectedBooking.status === "pending" && (
                  <div className="flex gap-3">
                    <button onClick={() => { setShowDetailsModal(false); setShowRejectModal(true); }}
                      className="px-6 py-2 border-2 border-red-600 text-red-600 rounded-lg hover:bg-red-50 transition font-medium">Reject</button>
                    <button onClick={() => showConfirmation(handleApprove, selectedBooking._id, "Approve Bike Booking", `Approve booking for ${selectedBooking.bike?.bikeName}?`, "approve")}
                      className="px-6 py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg transition font-medium">Approve</button>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="bg-white rounded-xl shadow-lg w-full max-w-4xl max-h-[85vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 p-6">
              <div className="flex justify-between items-center">
                ...
                <button onClick={() => setShowDocumentsModal(false)} className="text-gray-400 hover:text-gray-600"><FaTimesCircle size={24} /></button>
              </div>
              <p className="text-sm text-gray-500 mt-2">Verify customer identity documents before approving the booking</p>
            </div>
            <div className="p-6">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {[["citizenshipFront","Front Side"],["citizenshipBack","Back Side"]].map(([key, label]) =>
                    selectedDocuments[key] ? (
                      <div key={key} className="border rounded-xl p-4 bg-gray-50 hover:shadow-md transition">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-2">{getFileIcon(selectedDocuments[key].mimetype)}<p className="font-medium">{label}</p></div>
                          <button onClick={() => openImageViewer(`http://localhost:5000${selectedDocuments[key].url}`)}
                            className="text-blue-600 text-sm flex items-center gap-1"><FaExpand size={12} /> View Full</button>
                        </div>
                        {selectedDocuments[key].mimetype?.startsWith("image/") ? (
                          <div className="cursor-pointer overflow-hidden rounded-lg border border-gray-200"
                            onClick={() => openImageViewer(`http://localhost:5000${selectedDocuments[key].url}`)}>
                            <img src={`http://localhost:5000${selectedDocuments[key].url}`} alt={label}
                              className="w-full h-48 object-cover hover:scale-105 transition-transform duration-300" />
                          </div>
                        ) : (
                          <div className="p-8 bg-white rounded-lg border text-center">
                            <FaFilePdf className="text-red-500 text-5xl mx-auto mb-2" />
                            <a href={`http://localhost:5000${selectedDocuments[key].url}`} target="_blank" rel="noopener noreferrer"
                              className="px-4 py-2 bg-red-600 text-white rounded-lg text-sm hover:bg-red-700 inline-block mt-2">View PDF</a>
                          </div>
                        )}
                        <p className="text-xs text-gray-500 mt-2">Uploaded: {new Date(selectedDocuments[key].uploadedAt).toLocaleDateString()}</p>
                      </div>
                    ) : null
                  )}
                  {!selectedDocuments.citizenshipFront && !selectedDocuments.citizenshipBack && (
                    <div className="col-span-2 text-center py-8 bg-gray-50 rounded-lg">
                      <FaIdCard className="text-gray-400 text-5xl mx-auto mb-2" />
                      <p className="text-gray-500">No citizenship documents uploaded</p>
                    </div>
                  )}
                </div>
              </div>
              ...
              {(selectedDocuments.licenseFront || selectedDocuments.licenseBack) && (
                <div className="mb-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[["licenseFront","Front Side"],["licenseBack","Back Side"]].map(([key, label]) =>
                      selectedDocuments[key] ? (
                        <div key={key} className="border rounded-xl p-4 bg-gray-50 hover:shadow-md transition">
                          <div className="flex items-center justify-between mb-3">
                            <div className="flex items-center gap-2">{getFileIcon(selectedDocuments[key].mimetype)}<p className="font-medium">{label}</p></div>
              ...
              <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-4 border border-blue-200">
                <h4 className="font-semibold text-gray-800 mb-3 flex items-center gap-2">
                  <FaCheckCircle className="text-green-600" /> Verification Summary
                </h4>
                <div className="grid grid-cols-3 gap-4">
                  <div className="text-center p-3 bg-white rounded-lg">
                    <p className="text-sm text-gray-500">Citizenship</p>
                    <p className="font-semibold text-blue-600">{[selectedDocuments.citizenshipFront, selectedDocuments.citizenshipBack].filter(Boolean).length}/2 Uploaded</p>
                  </div>
                  <div className="text-center p-3 bg-white rounded-lg">
                    <p className="text-sm text-gray-500">Driver's License</p>
                    <p className="font-semibold text-green-600">{[selectedDocuments.licenseFront, selectedDocuments.licenseBack].filter(Boolean).length}/2 Uploaded</p>
                  </div>
                  <div className="text-center p-3 bg-white rounded-lg">
                    <p className="text-sm text-gray-500">Verification Status</p>
                    <p className={`font-semibold ${selectedDocuments.verificationStatus === "verified" ? "text-green-600" : selectedDocuments.verificationStatus === "rejected" ? "text-red-600" : "text-yellow-600"}`}>
      ...
      {showImageViewer && selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center p-4 z-[60]">
          <div className="relative max-w-5xl max-h-[90vh]">
            ...
            <img src={selectedImage} alt="Document Preview" className="max-w-full max-h-[85vh] object-contain rounded-lg shadow-2xl" />
            <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 bg-black/70 text-white px-4 py-2 rounded-lg text-sm">
              Click outside or press ESC to close
            </div>
          </div>
      ...
      {showRejectModal && selectedBooking && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md p-6">
            <div className="flex items-center gap-3 mb-4">
              <FaTimesCircle className="text-red-600 text-2xl" />
              <h3 className="text-xl font-bold text-gray-800">Reject Bike Booking</h3>
            </div>
            <p className="text-gray-600 mb-4">Reject booking <strong>{selectedBooking.confirmationCode}</strong>?</p>
            <div className="mb-4">
            ...
            <div className="flex justify-end gap-3">
              <button onClick={() => { setShowRejectModal(false); setRejectionReason(""); }}
                className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition">Cancel</button>
              <button onClick={() => handleReject(selectedBooking._id)} disabled={actionLoading}
                className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition font-medium disabled:opacity-50">
                {actionLoading ? <FaSpinner className="animate-spin inline" /> : "Reject Booking"}
              </button>
            </div>
          </div>
        </div>
