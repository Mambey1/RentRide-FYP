...
import googleLogo from "../../assets/Auth/google-logo.png";
import { useNavigate } from "react-router-dom";
import { FaCar, FaEye, FaEyeSlash, FaLock, FaEnvelope } from "react-icons/fa";
import { toast, ToastContainer } from "react-toastify";
import "react-toastify/dist/ReactToastify.css";
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (data.isBlocked) {
          toast.error(
            data.message || "Your account has been blocked. Please contact support."
          );
          setError(
            data.message || "Your account has been blocked. Please contact support."
          );
      ...
      // sessionStorage covers current tab session
      localStorage.setItem("token", data.token);
      localStorage.setItem("user", JSON.stringify(data.user));
      sessionStorage.setItem("token", data.token);
      sessionStorage.setItem("user", JSON.stringify(data.user));

      // If remember me is OFF, clear localStorage on tab close via a flag
      if (!remember) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="w-full max-w-5xl bg-white rounded-2xl shadow-xl flex overflow-hidden border border-gray-100 relative z-10 max-h-[80vh]">
        ...
        {/* Right Panel - Image */}
        <div className="w-1/2 relative overflow-hidden">
          <img src={loginImage} alt="Driving" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-black/20 to-transparent"></div>
          <div className="absolute bottom-0 left-0 right-0 p-6 text-white">
            <div className="bg-gradient-to-r from-white/10 to-transparent backdrop-blur-sm rounded-xl p-4 border border-white/20">
              <h3 className="text-lg font-bold mb-2">Drive Your Dream Car</h3>
