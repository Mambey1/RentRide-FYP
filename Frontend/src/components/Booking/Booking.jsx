...
// // //     try {
// // //       const response = await axiosInstance.get(`/profile/user/${userId}`);
// // //       if (response.data.success) setOwnerDetails(response.data.user);
// // //     } catch (err) {
...
// // //         ),
// // //       );
// // //       const revert = new Set(likedReviews);
// // //       wasLiked ? revert.add(reviewId) : revert.delete(reviewId);
// // //       setLikedReviews(revert);
// // //       localStorage.setItem("likedReviews", JSON.stringify([...revert]));
...
// // //             image = `https://ui-avatars.com/api/?background=6B21A5&color=fff&rounded=true&size=128&name=Support`;
// // //           } else {
// // //             role = other.role === "admin" ? "admin" : "user";
// // //             displayName = other.name;
// // //             image = other.profilePhoto
// // //               ? `http://localhost:5000/uploads/profiles/${other.profilePhoto}`
// // //               : `https://ui-avatars.com/api/?background=3B82F6&color=fff&rounded=true&size=128&name=${displayName.charAt(0)}`;
...
// // //                       type="button"
// // //                       onClick={() => setInsuranceOption("basic")}
// // //                       className={`p-4 rounded-xl border-2 text-left transition-all ${insuranceOption === "basic" ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-gray-300"}`}
// // //                     >
// // //                       <div className="font-semibold text-gray-900">
// // //                         Basic Coverage
// // //                       </div>
...
// // //                       type="button"
// // //                       onClick={() => setInsuranceOption("premium")}
// // //                       className={`p-4 rounded-xl border-2 text-left transition-all ${insuranceOption === "premium" ? "border-green-500 bg-green-50" : "border-gray-200 hover:border-gray-300"}`}
// // //                     >
// // //                       <div className="font-semibold text-gray-900">
// // //                         Premium Coverage
// // //                       </div>
...
// // //                     placeholder="Any specific requirements or notes for the owner..."
// // //                   />
// // //                 </div>
// // //               </div>
// // //             </div>
...
// // //                     setShowReviewForm(true);
// // //                   }}
// // //                   className="w-full py-3 mb-4 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl transition flex items-center justify-center gap-2"
// // //                 >
...
// // //                       }
// // //                       disabled={submittingReview}
// // //                       className="flex-1 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition flex items-center justify-center gap-2"
// // //                     >
// // //                       {submittingReview ? (
// // //                         <FaSpinner className="animate-spin" />
// // //                       ) : (
...
// // //                         <button
// // //                           onClick={(e) => handleToggleHelpful(review._id, e)}
// // //                           className={`flex items-center gap-1.5 text-xs px-3 py-1 rounded-full transition ${review.hasLiked ? "bg-red-50 text-red-500" : "bg-gray-100 text-gray-500 hover:bg-gray-200"}`}
// // //                         >
// // //                           {review.hasLiked ? (
// // //                             <FaHeart size={10} />
// // //                           ) : (
...
// // //                         <button
// // //                           onClick={() => handleOpenReviewerChat(review.user)}
// // //                           className="flex items-center gap-1.5 text-xs px-3 py-1 bg-purple-50 text-purple-600 hover:bg-purple-100 rounded-full transition"
// // //                         >
...
const getVehicleStatusInfo = (vehicle) => {
  ...
  if (status === "On Hold")
    return { type: "onHold", label: "On Hold", color: "amber" };
  if (status === "Booked" || status === "booked")
    return { type: "booked", label: "Booked", color: "red" };
  if (status === "Available")
    return { type: "available", label: "Available", color: "green" };
  return { type: "unavailable", label: status, color: "gray" };
};
...
const Booking = () => {
      ...
      async () => {
        ...
        try {
          ...
          if (response.data.success) {
            ...
            setShowReviewForm(false);
            setEditingReview(false);
            // ── Reset local state completely so user can re-submit ──────────
            setNewComment("");
            setNewRating(5);
            await fetchReviews();
  ...
  const statusInfo = getVehicleStatusInfo(vehicleDetails);
  const vehicleIsOnHold = statusInfo?.type === "onHold";
  const vehicleIsBooked = statusInfo?.type === "booked";
  const vehicleIsUnavailable = vehicleIsOnHold || vehicleIsBooked;
  ...
  const handleBookNow = async () => {
    ...
    if (vehicleIsUnavailable) {
      toast.error(
        vehicleIsOnHold
          ? "This vehicle is on hold. Please wait until it becomes available."
          : "This vehicle is currently booked. Please choose another vehicle.",
      );
      return;
    }
    if (!selectedDate) {
    ...
    try {
      ...
      if (!response.data?.data?.booking?.id)
        throw new Error("No booking ID received from server");
      toast.success("Booking created! Redirecting to document upload...");
      setTimeout(() => {
        navigate("/upload-documents", {
          state: {
            bookingId: response.data.data.booking.id,
          ...
          <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-amber-800 font-semibold text-lg">
                        This vehicle is currently on hold
                      </p>
                      <p className="text-amber-700 text-sm mt-0.5">
                        Another customer has 24 hours to complete payment.{" "}
                        {vehicleDetails.holdExpiresAt && (
                          <strong>
                            Available in:{" "}
                            {getHoldTimeRemaining(vehicleDetails.holdExpiresAt)}
                          </strong>
                        )}{" "}
                  ...
                  <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-xl flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-red-700 text-sm mt-0.5">
                        This vehicle is already booked by another customer.
            ...
            {!vehicleIsUnavailable ? (
              ...
              >
                ...
                <h3
                  className={`text-2xl font-bold mb-3 ${vehicleIsOnHold ? "text-amber-800" : "text-red-800"}`}
                >
                  {vehicleIsOnHold
                    ? "Vehicle Currently On Hold"
                    : "Vehicle Currently Booked"}
                </h3>
                <p
                  className={`mb-2 ${vehicleIsOnHold ? "text-amber-700" : "text-red-700"}`}
                >
                  {vehicleIsOnHold
                    ? "This vehicle is reserved for another customer who is completing their payment."
                    : "This vehicle is currently booked by another customer and is not available."}
                </p>
                {vehicleIsOnHold && vehicleDetails.holdExpiresAt && (
                  <p className="text-amber-800 font-semibold text-lg mt-2">
                ...
                <p
                  className={`text-sm mt-4 ${vehicleIsOnHold ? "text-amber-600" : "text-red-600"}`}
                >
                  {vehicleIsOnHold
                    ? "The vehicle will be automatically released if payment is not completed within the hold period."
          ...
          <div className="space-y-8">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 sticky top-24">
              ...
              <div className="space-y-4">
                ...
                <button
                  onClick={handleBookNow}
                  disabled={submitting || vehicleIsUnavailable}
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              {/* ── Review CTA based on booking status ── */}
              {!hasBookedThisVehicle && !userReview && !showReviewForm && (
                <div className="mb-4 p-4 bg-gray-50 border border-gray-200 rounded-xl">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-0.5">
                        You must book and complete a rental of this vehicle to
                        write a review. Reviews help other customers make better
                        decisions.
                      </p>
                      <button
                        onClick={() => {
                        ...
                        className="mt-2 px-4 py-1.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition"
                      >
                        {vehicleIsUnavailable
                  ...
                  {reviews.map((review) => (
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between mt-3">
                        <button
                          onClick={(e) => handleToggleHelpful(review._id, e)}
                          className={`flex items-center gap-1.5 text-xs px-3 py-1 rounded-full transition ${
                            review.hasLiked
