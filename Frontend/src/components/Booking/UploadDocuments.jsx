  ...
  return (
    <div style={{ marginBottom: 12 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
        <span style={{ fontSize: 13, fontWeight: 600, color: clr.navy, letterSpacing: "0.01em" }}>
          {label}
        </span>
        {required && (
          <span style={{
            fontSize: 10, fontWeight: 700, color: clr.blue,
            background: clr.blueBg, padding: "2px 7px", borderRadius: 20,
      ...
      <input type="file" accept="image/*,.pdf" onChange={onChange} id={id} style={{ display: "none" }} />
      <label
        htmlFor={id}
      ...
      >
        <div style={{
          ...
          {file
            ...
            : <FaCloudUploadAlt style={{ color: file || dragging ? "#fff" : clr.slate, fontSize: 16 }} />
          }
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          {file ? (
            <>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: clr.blueDark, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                {file.name}
              </p>
            ...
            <>
              <p style={{ margin: 0, fontSize: 13, fontWeight: 500, color: clr.slate }}>
                Drop file here or <span style={{ color: clr.blue, fontWeight: 600 }}>browse</span>
              </p>
              <p style={{ margin: "2px 0 0", fontSize: 11, color: clr.slateLight }}>
        ...
        {file && (
          <FaCheckCircle style={{ color: clr.green, fontSize: 18, flexShrink: 0 }} />
        )}
      </label>
    </div>
  );
...
const SectionCard = ({ icon: Icon, title, badge, children }) => (
  <div style={{
    ...
    <div style={{
      ...
      <span style={{ fontSize: 14, fontWeight: 700, color: clr.navy, flex: 1 }}>{title}</span>
      {badge && (
        <span style={{
          fontSize: 10, fontWeight: 700, letterSpacing: "0.06em",
          textTransform: "uppercase", color: clr.blue,
          background: clr.blueBg, padding: "3px 9px", borderRadius: 20,
        ...
        <React.Fragment key={s}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
            <div style={{
              ...
              background: i < current ? clr.blue : i === current ? clr.blue : "#E2E8F0",
              ...
              {i < current
                ...
                : <span style={{ fontSize: 11, fontWeight: 700, color: i === current ? "#fff" : clr.slateLight }}>{i + 1}</span>
              }
            </div>
            <span style={{ fontSize: 11, fontWeight: i === current ? 700 : 400, color: i === current ? clr.navy : clr.slateLight, whiteSpace: "nowrap" }}>
              {s}
            </span>
          </div>
          {i < steps.length - 1 && (
...
const UploadDocuments = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { bookingId, confirmationCode, vehicleDetails, driverOption, bookingDetails } = location.state || {};
  ...
  const completedDocs = [citizenshipFront, citizenshipBack, isSelfDrive && licenseFront, isSelfDrive && licenseBack]
  ...
  if (!bookingId) {
    return (
      <div style={{ minHeight: "100vh", background: clr.pageBg, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center", padding: 32, background: clr.surface, borderRadius: 16, border: `1px solid ${clr.border}` }}>
          <FaTimesCircle style={{ color: clr.red, fontSize: 40, marginBottom: 12 }} />
          <p style={{ color: clr.red, fontWeight: 600, marginBottom: 16 }}>No booking found. Please start over.</p>
          <button onClick={() => navigate("/rentridehome")} style={{
            padding: "10px 24px", background: clr.blue, color: "#fff",
  ...
  const handleFileChange = (e, setFile, fileType) => {
    const file = e.target.files[0];
    if (!file) return;
    if (file.size > 5 * 1024 * 1024) { setError(`${fileType} must be under 5 MB`); return; }
    setFile(file);
    setError("");
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!citizenshipFront || !citizenshipBack) { setError("Please upload both sides of your citizenship certificate."); return; }
    if (isSelfDrive && (!licenseFront || !licenseBack)) { setError("Self-drive requires both sides of your driver's license."); return; }
    if (!agreedToContract) { setError("Please accept the contract terms to proceed."); return; }
    ...
    try {
      ...
      if (response.data.success) { setUploaded(true); setShowSuccessModal(true); }
    } catch (err) {
  ...
  return (
    <div style={{ minHeight: "100vh", background: clr.pageBg, fontFamily: "'Inter', -apple-system, sans-serif" }}>
      {/* Top bar */}
      <div style={{
        ...
        <button onClick={() => navigate(-1)} style={{
          ...
          color: clr.slate, fontSize: 13, fontWeight: 500, padding: "6px 10px",
        ...
        <span style={{ fontSize: 14, fontWeight: 700, color: clr.navy }}>Document Upload</span>
        {confirmationCode && (
          <span style={{
            marginLeft: "auto", fontSize: 12, fontWeight: 600,
            color: clr.blue, background: clr.blueBg,
      ...
      <div style={{ maxWidth: 680, margin: "0 auto", padding: "36px 20px 60px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ margin: "0 0 6px", fontSize: 26, fontWeight: 800, color: clr.navy, letterSpacing: "-0.02em" }}>
            Upload Documents
          </h1>
        ...
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <p style={{ margin: "0 0 2px", fontWeight: 700, fontSize: 15, color: "#fff" }}>
          ...
          <div style={{ textAlign: "right", flexShrink: 0 }}>
            <p style={{ margin: "0 0 2px", fontWeight: 800, fontSize: 18, color: "#fff" }}>
              रु {bookingDetails?.totalAmount?.toLocaleString()}
            </p>
            <span style={{
              fontSize: 11, fontWeight: 700, color: clr.navy,
        ...
        <div style={{
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ fontSize: 12, fontWeight: 600, color: clr.slate }}>Upload progress</span>
              <span style={{ fontSize: 12, fontWeight: 700, color: clr.blue }}>{completedDocs}/{totalDocs} files</span>
            </div>
          ...
          {progress === 100 && (
            <FaCheckCircle style={{ color: clr.green, fontSize: 20, flexShrink: 0 }} />
          )}
        </div>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", gap: 12 }}>
            <FaExclamationTriangle style={{ color: clr.amber, fontSize: 15, marginTop: 2, flexShrink: 0 }} />
            <div>
              <p style={{ margin: "0 0 4px", fontWeight: 700, fontSize: 13, color: "#92400E" }}>
                The document holder must be present at pickup
              </p>
              <ul style={{ margin: 0, paddingLeft: 16, fontSize: 12, color: "#A16207", lineHeight: 1.7 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          ...
          {/* Citizenship */}
          <SectionCard icon={FaIdCard} title="Citizenship Certificate" badge="Always Required">
            <div style={{
              display: "grid",
              gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))",
              gap: 12
            }}>
              <FileUploadZone label="Front Side" id="citizenshipFront" file={citizenshipFront}
                onChange={(e) => handleFileChange(e, setCitizenshipFront, "Citizenship front")} required />
              <FileUploadZone label="Back Side" id="citizenshipBack" file={citizenshipBack}
                onChange={(e) => handleFileChange(e, setCitizenshipBack, "Citizenship back")} required />
            </div>
          </SectionCard>

          {/* License */}
          {isSelfDrive && (
            <SectionCard icon={FaPassport} title="Driver's License" badge="Self-Drive Required">
              <div style={{
                display: "grid",
                gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))",
                gap: 12
              }}>
                <FileUploadZone label="Front Side" id="licenseFront" file={licenseFront}
                  onChange={(e) => handleFileChange(e, setLicenseFront, "License front")} required />
                <FileUploadZone label="Back Side" id="licenseBack" file={licenseBack}
                  onChange={(e) => handleFileChange(e, setLicenseBack, "License back")} required />
              </div>
            </SectionCard>
          )}
          ...
          <SectionCard icon={FaFileSignature} title="Rental Agreement">
            <div style={{ marginBottom: 16 }}>
              {[
                ...
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                  ...
                  <p style={{ margin: 0, fontSize: 13, color: clr.slate, lineHeight: 1.5 }}>{term}</p>
                </div>
              ))}
            </div>
            ...
            <label style={{
              ...
              <input type="checkbox" checked={agreedToContract}
                onChange={(e) => setAgreedToContract(e.target.checked)}
                style={{ width: 17, height: 17, accentColor: clr.blue, cursor: "pointer", flexShrink: 0 }}
              />
              <span style={{ fontSize: 13, fontWeight: 600, color: agreedToContract ? "#15803D" : clr.navy }}>
                I confirm and agree to sign the contract at the time of pickup
              </span>
              {agreedToContract && <FaCheckCircle style={{ color: clr.green, fontSize: 16, marginLeft: "auto", flexShrink: 0 }} />}
            </label>
          </SectionCard>
          ...
          {error && (
            <div style={{
              ...
              background: clr.redBg, border: `1px solid ${clr.redBorder}`,
              borderRadius: 10, padding: "12px 16px",
            }}>
              <FaExclamationTriangle style={{ color: clr.red, fontSize: 14, marginTop: 2, flexShrink: 0 }} />
              <p style={{ margin: 0, fontSize: 13, color: clr.red, fontWeight: 500 }}>{error}</p>
            </div>
          )}
          ...
          <button
            ...
            style={{
              ...
              fontSize: 15, fontWeight: 700, cursor: uploading || uploaded ? "not-allowed" : "pointer",
              display: "flex", alignItems: "center", justifyContent: "center", gap: 10,
              boxShadow: uploading || uploaded ? "none" : "0 4px 14px rgba(59,130,246,0.35)",
              transition: "all 0.2s",
              letterSpacing: "0.01em",
            }}
          >
            {uploading ? (
              <><FaSpinner style={{ animation: "spin 1s linear infinite" }} /> Uploading documents…</>
            ) : (
              <><FaShieldAlt /> Submit for Verification</>
            )}
          </button>

          <p style={{ textAlign: "center", fontSize: 12, color: clr.slateLight, margin: "4px 0 0" }}>
      ...
      {showSuccessModal && (
        <div style={{
          ...
          display: "flex", alignItems: "center", justifyContent: "center",
          zIndex: 50, padding: 20, backdropFilter: "blur(3px)",
        }}>
          <div style={{
            background: clr.surface, borderRadius: 20,
            maxWidth: 440, width: "100%", overflow: "hidden",
            boxShadow: "0 25px 60px rgba(0,0,0,0.18)",
            animation: "fadeUp 0.3s ease-out",
          }}>
            {/* Green top accent */}
            <div style={{ height: 4, background: `linear-gradient(90deg, ${clr.green}, #34D399)` }} />

            <div style={{ padding: "32px 32px 28px", textAlign: "center" }}>
              <div style={{
                width: 64, height: 64, borderRadius: "50%",
                background: clr.greenBg, border: `2px solid ${clr.greenBorder}`,
                display: "flex", alignItems: "center", justifyContent: "center",
                margin: "0 auto 20px",
              }}>
              ...
              <h2 style={{ margin: "0 0 8px", fontSize: 20, fontWeight: 800, color: clr.navy, letterSpacing: "-0.02em" }}>
                Documents Submitted
              </h2>
              <p style={{ margin: "0 0 24px", fontSize: 14, color: clr.slate, lineHeight: 1.6 }}>
                Your booking is pending verification. We'll notify you once your documents are reviewed.
              </p>

              <div style={{
                background: clr.pageBg, border: `1px solid ${clr.border}`,
                borderRadius: 12, padding: "16px 18px", marginBottom: 24, textAlign: "left",
              }}>
                <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                  <FaClock style={{ color: clr.blue, fontSize: 15, marginTop: 2, flexShrink: 0 }} />
                  <div>
                    <p style={{ margin: "0 0 8px", fontSize: 13, fontWeight: 700, color: clr.navy }}>What happens next?</p>
                    {[
                      ...
                      <div key={i} style={{ display: "flex", gap: 8, marginBottom: i < 2 ? 6 : 0 }}>
                        <span style={{ fontSize: 12, fontWeight: 700, color: clr.blue }}>0{i + 1}</span>
                        <span style={{ fontSize: 12, color: clr.slate }}>{step}</span>
                      </div>
                    ))}
                  </div>
                </div>
