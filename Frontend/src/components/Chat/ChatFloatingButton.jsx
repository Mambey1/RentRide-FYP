...
// KEY HELPER: reads unreadCounts regardless of ObjectId vs string key format
const getUnreadForUser = (chat, userId) => {
  if (!userId || !chat.unreadCounts) return 0;
  const uid = userId.toString();
  // Try direct key first
  if (chat.unreadCounts[uid] !== undefined) return chat.unreadCounts[uid] || 0;
  // Scan all keys with toString() comparison (handles MongoDB ObjectId serialization)
    ...
    >
      ...
      >
        ...
        <div className="relative max-w-[75%]">
          ...
          <div
            className={`px-3 py-2 text-sm leading-relaxed break-words rounded-2xl ${isUnsent ? "bg-gray-100 text-gray-400 italic border border-gray-200" : isOwn ? "bg-gradient-to-br from-blue-600 to-purple-600 text-white rounded-br-sm" : "bg-white text-gray-800 border border-gray-100 rounded-bl-sm shadow-sm"}`}
          >
            {isUnsent ? (
              <span className="flex items-center gap-1.5">
                <FaTimes size={10} /> This message was unsent
            ...
            >
              {Object.entries(reactionGroups).map(([emoji, count]) => (
                <button
                  key={emoji}
                  onClick={() => onReact(msg._id, emoji)}
                  className={`flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[11px] border transition ${myReaction?.emoji === emoji ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-white border-gray-200 text-gray-600 hover:bg-gray-50"}`}
                >
                  {emoji}
...
const InlineChatWindow = ({
  ...
  const { onNewMessage, joinChat, leaveChat, markRead, socket } = useSocket();
  ...
  useEffect(() => {
    const unsub = onNewMessage(chat._id, (data) => {
      ...
      const sid = (data.senderId || data.message?.sender?._id)?.toString();
      const isOwn = !!(uid && sid && uid === sid);
      if (!isOwn) {
        setMessages((prev) => {
          if (prev.some((m) => m._id === data.message._id)) return prev;
          return [...prev, data.message];
  ...
  useEffect(() => {
    ...
    const h2 = ({ messageId, reactions }) =>
      setMessages((prev) =>
        prev.map((m) => (m._id === messageId ? { ...m, reactions } : m)),
      );
    socket.on("message_unsent", h1);
    socket.on("message_reaction", h2);
    return () => {
      socket.off("message_unsent", h1);
      socket.off("message_reaction", h2);
    };
  }, [socket]);
  ...
  const sendMessage = async () => {
    ...
    const temp = {
      ...
      replyTo: replyTo
        ...
        : null,
      sender: { _id: cu.id, name: cu.name, profilePhoto: cu.profilePhoto },
    };
    setMessages((prev) => [...prev, temp]);
    scrollToBottom();
    try {
...
const ChatListPanel = ({
  ...
  // Get userId as string — critical for matching MongoDB ObjectId keys
  const currentUser = JSON.parse(
    localStorage.getItem("user") || sessionStorage.getItem("user") || "{}",
  );
  ...
  // ── REAL-TIME: update chat list when new_message arrives ──────────────────
  useEffect(() => {
      ...
      setChats((prev) => {
        ...
        return prev
          .map((chat) => {
            ...
            const currentUnread = getUnreadForUser(chat, userId);
            const newUnread = isOwn ? 0 : currentUnread + 1;
            // Spread unreadCounts safely — may be plain object from MongoDB
            const counts = { ...(chat.unreadCounts || {}) };
            counts[userId] = newUnread;
            return {
              ...chat,
              lastMessage:
  ...
  // ── REAL-TIME: also re-fetch on notification (safety net) ─────────────────
  useEffect(() => {
    if (!onNewMessageNotification) return;
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          filtered.map((chat) => {
            ...
            const isSupportChat = chat.chatType === "support";
            // Use the helper — handles ObjectId key mismatch
            const unread = getUnreadForUser(chat, userId);
            const isMuted = chat.isMuted;
            return (
              <div
                key={chat._id}
