...
// // import { useNavigate } from "react-router-dom";
// // import {
// //   FaBell, FaCheckCircle, FaTimesCircle, FaInfoCircle, FaCar,
// //   FaMotorcycle, FaEnvelope, FaTimes, FaTrash, FaWallet,
// //   FaCommentAlt, FaCheck, FaCheckDouble,
// // } from "react-icons/fa";
// // import { useNotifications } from "../../hooks/useNotifications";
...
// //   const src = buildSrc(photo);
// //   if (src && !imgError) {
// //     return <img src={src} alt={name} className={`${size} rounded-full object-cover ring-2 ring-white shadow-md flex-shrink-0`} onError={() => setImgError(true)} />;
// //   }
// //   return (
// //     <div className={`${size} rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center flex-shrink-0 ring-2 ring-white shadow-md`}>
// //       <span className="text-white text-sm font-bold">{initials}</span>
...
// //   const dismiss = (e) => { if (e) e.stopPropagation(); setExiting(true); setTimeout(() => onDismiss(toast.id), 280); };
// //   const handleReply = (e) => { e.stopPropagation(); setExiting(true); setTimeout(() => { onDismiss(toast.id); onNavigate(toast.chatId); }, 150); };
// //   return (
// //     <div className={`relative w-[320px] bg-white rounded-2xl overflow-hidden transition-all duration-[280ms] ease-in-out ${exiting ? "opacity-0 translate-x-full scale-95" : "opacity-100 translate-x-0 scale-100"}`}
// //       style={{ boxShadow: "0 8px 32px rgba(99,102,241,0.22), 0 2px 8px rgba(0,0,0,0.10)", animation: exiting ? "none" : "toastSlideIn 0.38s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
// //       <div className="h-[3px] w-full bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500" />
// //       <div className="px-4 pt-3 pb-3">
// //         <div className="flex items-center gap-3">
// //           <div className="relative flex-shrink-0">
...
// //                 <span className="flex-shrink-0 text-[8px] font-semibold text-green-600 bg-green-50 border border-green-100 px-1.5 py-0.5 rounded-full">online</span>
// //               </div>
// //               <button onClick={dismiss} className="flex-shrink-0 w-5 h-5 rounded-full bg-gray-100 hover:bg-red-50 hover:text-red-400 text-gray-400 flex items-center justify-center transition-colors"><FaTimes size={8} /></button>
// //             </div>
// //             {toast.vehicleName && <p className="text-[10px] text-indigo-500 font-medium flex items-center gap-1 mt-0.5"><FaCar size={8} /><span className="truncate">{toast.vehicleName}</span></p>}
// //           </div>
// //         </div>
// //         <div className="mt-2 ml-[48px] px-2.5 py-1.5 bg-gray-50 border border-gray-100 rounded-xl rounded-tl-sm">
// //           <p className="text-xs text-gray-700 line-clamp-2 leading-relaxed">{toast.message}</p>
// //         </div>
// //         <button onClick={handleReply} className="mt-2 ml-[48px] w-[calc(100%-48px)] flex items-center justify-center gap-1.5 py-1.5 bg-gradient-to-r from-indigo-500 to-purple-600 hover:from-indigo-600 hover:to-purple-700 text-white text-[12px] font-semibold rounded-lg transition-all duration-150 shadow-sm hover:shadow-md active:scale-[0.97]">
// //           <FaCommentAlt size={9} /> Reply to {toast.from.split(" ")[0]}
// //         </button>
// //       </div>
// //       <div className="absolute bottom-0 left-0 h-[3px] bg-gradient-to-r from-indigo-400 to-purple-500" style={{ animation: "toastShrink 5s linear forwards" }} />
// //       <style>{`
...
// //   const { notifications, unreadCount, loading, markAsRead, markAllAsRead, deleteNotification, fetchNotifications } = useNotifications(socket);
...
// //   const getIcon = (type, action) => {
// //     const isBikePayment = action?.type === "bike_payment";
// //     if (isBikePayment)
// //       return <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center"><FaMotorcycle className="text-purple-600 text-xs" /></div>;
// //     if (action?.type === "payment" || type === "payment")
// //       return <div className="w-8 h-8 rounded-full bg-purple-100 flex items-center justify-center"><FaWallet className="text-purple-600 text-xs" /></div>;
// //     if (type === "chat")
// //       return <div className="w-8 h-8 rounded-full bg-indigo-100 flex items-center justify-center"><FaCommentAlt className="text-indigo-600 text-xs" /></div>;
...
// //     const map = {
// //       payment: "border-l-4 border-l-purple-400 bg-purple-50",
// //       bike_payment: "border-l-4 border-l-purple-400 bg-purple-50",
// //       chat:    "border-l-4 border-l-indigo-400 bg-indigo-50",
// //       booking: "border-l-4 border-l-blue-400 bg-blue-50",
// //       success: "border-l-4 border-l-green-400 bg-green-50",
// //       error:   "border-l-4 border-l-red-400 bg-red-50",
// //       warning: "border-l-4 border-l-amber-400 bg-amber-50",
// //     };
// //     const key = action?.type === "bike_payment" ? "bike_payment" : action?.type === "payment" ? "payment" : type;
// //     return map[key] || "border-l-4 border-l-gray-300 bg-gray-50";
// //   };
...
// //     // 🏍️ Bike payment notification
// //     if (action?.type === "bike_payment") {
// //       if (metadata?.bookingId) {
// //         sessionStorage.setItem("current_bike_booking_id", String(metadata.bookingId));
// //         navigate(`/bike-payment/${metadata.bookingId}`);
// //       }
// //     // 🚗 Vehicle payment notification
...
// //       navigate(action.path);
// //     } else if (metadata?.bookingId) {
// //       // Fallback — check bookingType in metadata
// //       if (metadata?.bookingType === "bike") {
// //         sessionStorage.setItem("current_bike_booking_id", String(metadata.bookingId));
// //         navigate(`/bike-payment/${metadata.bookingId}`);
// //       } else {
// //         navigate(`/payment/${metadata.bookingId}`);
...
// //           <div className="px-5 py-4 bg-gradient-to-r from-indigo-600 to-purple-600 flex items-center justify-between">
// //             <div>
// //               <h3 className="text-base font-bold text-white flex items-center gap-2"><FaBell className="text-white/80" /> Notifications</h3>
// //               <p className="text-xs text-white/70 mt-0.5">{unreadCount > 0 ? `${unreadCount} unread` : "All caught up!"}</p>
// //             </div>
// //             <div className="flex items-center gap-2">
// //               {unreadCount > 0 && (
// //                 <button onClick={markAllAsRead} className="text-xs text-white/80 hover:text-white bg-white/10 hover:bg-white/20 px-2.5 py-1 rounded-full flex items-center gap-1 transition-colors">
...
// //                 </button>
// //               )}
// //               <button onClick={() => setIsOpen(false)} className="text-white/70 hover:text-white bg-white/10 hover:bg-white/20 p-1.5 rounded-full transition-colors"><FaTimes size={12} /></button>
// //             </div>
// //           </div>
...
// //                   <div className="flex-1 min-w-0">
// //                     <div className="flex items-start justify-between gap-2">
// //                       <p className={`text-sm leading-tight ${!n.read ? "font-semibold text-gray-900" : "font-medium text-gray-600"}`}>{n.title}</p>
// //                       <span className="text-[10px] text-gray-400 flex-shrink-0 mt-0.5">{formatTime(n.createdAt)}</span>
// //                     </div>
// //                     <p className="text-xs text-gray-500 mt-0.5 leading-relaxed line-clamp-2">{n.message}</p>
// //                     {n.metadata?.amount && (
...
// //                     )}
// //                     {/* Bike payment badge */}
// //                     {n.action?.type === "bike_payment" && (
// //                       <span className="inline-flex items-center gap-1 mt-1 text-[10px] font-semibold text-purple-600 bg-purple-50 border border-purple-100 px-2 py-0.5 rounded-full">
// //                         <FaMotorcycle size={8} /> Bike Booking
// //                       </span>
// //                     )}
// //                   </div>
// //                   <div className="flex flex-col items-center gap-2 flex-shrink-0">
// //                     {!n.read && <span className="w-2 h-2 rounded-full bg-indigo-500" />}
// //                     <button onClick={(e) => { e.stopPropagation(); deleteNotification(n._id || n.id); }}
// //                       className="text-gray-300 hover:text-red-400 transition-colors p-0.5 rounded hover:bg-red-50">
// //                       <FaTrash size={10} />
// //                     </button>
// //                   </div>
...
// //             ) : (
// //               <div className="py-14 text-center">
// //                 <div className="w-14 h-14 rounded-full bg-gray-100 flex items-center justify-center mx-auto mb-4"><FaBell className="text-2xl text-gray-300" /></div>
// //                 <p className="text-sm font-medium text-gray-600">No notifications yet</p>
// //                 <p className="text-xs text-gray-400 mt-1">We'll let you know when something happens</p>
// //               </div>
// //             )}
...
//   return (
//     <div
//       className={`relative w-[320px] bg-white rounded-2xl overflow-hidden transition-all duration-[280ms] ease-in-out ${exiting ? "opacity-0 translate-x-full scale-95" : "opacity-100 translate-x-0 scale-100"}`}
//       style={{
//         boxShadow:
//           "0 8px 32px rgba(99,102,241,0.22), 0 2px 8px rgba(0,0,0,0.10)",
//         animation: exiting
...
const Notification = ({ socket }) => {
  ...
  const handleNotificationClick = async (n) => {
    ...
    // 🏍️ Bike payment — now uses same PaymentComponent as vehicles
    if (action?.type === "bike_payment") {
      if (metadata?.bookingId) {
        navigate(`/payment/${metadata.bookingId}`);
      }
