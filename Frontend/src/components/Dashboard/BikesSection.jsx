...
// //                             ? ` License: ${bike.licenseRequired}.`
// //                             : ""}
// //                           {bike.securityDeposit > 0
// //                             ? ` Deposit: &#2352;${bike.securityDeposit}.`
// //                             : ""}{" "}
// //                           Full price breakdown shown during booking.
// //                         </p>
// //                       </div>
// //                     </div>
// //                   </div>
...
// //                       Free cancellation &bull; 24/7 support &bull; Instant
// //                       confirmation
// //                     </p>
// //                   </div>
// //                 </div>
...
// // const BikesSection = ({ isLoggedIn = false, embeddedMode = false }) => {
...
// import axios from "axios";
// import {
//   FaMotorcycle, FaSearch, FaTachometerAlt, FaBolt,
//   FaShieldAlt, FaArrowRight, FaPhone, FaGasPump, FaCogs, FaCalendarAlt,
// } from "react-icons/fa";

// const API_URL = "http://localhost:5000/api";

// const getBikeImage = (bike, index = 0) => {
//   if (bike.photos?.length > index) return `http://localhost:5000/uploads/bikes/${bike.photos[index].filename}`;
//   return null;
// };
...
//               <div className="flex items-center gap-2 mt-2">
//                 <span className="px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-xs font-semibold border border-blue-100">{bike.bikeType}</span>
//                 <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusStyle(bike.status)}`}>{bike.status}</span>
//                 {bike.bikeNumber && <span className="text-gray-400 text-xs">#{bike.bikeNumber}</span>}
//               </div>
//             </div>
//             <button onClick={onClose} className="w-10 h-10 flex items-center justify-center bg-gray-100 hover:bg-red-50 hover:text-red-500 rounded-full transition-all duration-200 text-gray-500 text-xl font-light">×</button>
//           </div>
//           <div className="p-8">
//             <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
...
//                     <div className="absolute bottom-4 left-4 bg-black/60 text-white px-3 py-1.5 rounded-full text-sm backdrop-blur-sm">{photo.label}</div>
//                   </div>
//                 </div>
//               )) : (
//                 <div className="lg:col-span-3 h-80 bg-gradient-to-br from-gray-100 to-gray-200 rounded-2xl flex items-center justify-center">
...
//                     { icon: FaGasPump, label: "Fuel Type", value: bike.fuelType },
...
//                       <div className="p-3 bg-white rounded-lg shadow-sm mr-4"><spec.icon className="text-blue-600" /></div>
//                       <div><p className="text-sm text-gray-500">{spec.label}</p><p className="font-semibold text-gray-900">{spec.value}</p></div>
//                     </div>
//                   ))}
//                 </div>
//                 {bike.features?.length > 0 && (
...
//                     <div className="flex flex-wrap gap-3">
//                       {bike.features.map((f, i) => <span key={i} className="px-4 py-2 bg-blue-50 text-blue-700 rounded-lg font-medium">{f}</span>)}
//                     </div>
//                   </div>
//                 )}
//                 {bike.description && (
...
//                     <p className="text-gray-500 mb-2">Daily Rate</p>
//                     <div className="flex items-baseline justify-center">
//                       <span className="text-5xl font-bold text-gray-900">&#2352;{bike.ratePerDay}</span>
//                       <span className="text-gray-500 ml-2">/day</span>
//                     </div>
//                     {bike.ratePerWeek && <p className="text-sm text-blue-600 mt-2">Weekly: &#2352;{bike.ratePerWeek}</p>}
//                   </div>
//                   <div className="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-lg">
//                     <div className="flex items-start">
//                       <FaShieldAlt className="text-blue-600 mr-3 mt-0.5 flex-shrink-0" />
...
//                         {bike.minimumAge ? ` Min. age ${bike.minimumAge} yrs.` : ""}
//                         {bike.securityDeposit > 0 ? ` Deposit: &#2352;${bike.securityDeposit}.` : ""} Full price breakdown during booking.
//                       </p>
//                     </div>
//                   </div>
//                   {bike.status === "Available" ? (
...
//                   )}
//                   <div className="mt-6 text-center">
//                     <p className="text-gray-500 text-sm">Free cancellation • 24/7 support • Instant confirmation</p>
//                   </div>
//                 </div>
//               </div>
//             </div>
...
// // ── Main BikesSection ─────────────────────────────────────────────────────────
// const BikesSection = ({ isLoggedIn = false, embeddedMode = false, onCountReady = null }) => {
//   const navigate = useNavigate();
//   const [bikes, setBikes] = useState([]);
//   const [filtered, setFiltered] = useState([]);
//   const [loading, setLoading] = useState(true);
...
//         setBikes(res.data.data);
//         setFiltered(res.data.data);
//         if (onCountReady) onCountReady(res.data.data.length); // ← sends count to parent
//       }
//     } catch (err) {
//       console.error("Failed to fetch bikes:", err);
//     } finally {
...
//             <FaMotorcycle /> Bike Rentals
//           </span>
//           <h2 className="text-3xl sm:text-4xl font-black text-gray-900 mb-3">Ride on Two Wheels</h2>
//           <p className="text-gray-500 max-w-xl mx-auto">From nimble scooters to powerful sports bikes — find your perfect ride in Kathmandu.</p>
//         </div>
//       )}
...
//         <div className="relative flex-1">
//           <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
//           <input type="text" placeholder="Search bikes..." value={search} onChange={(e) => setSearch(e.target.value)}
//             className="w-full pl-11 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm" />
//         </div>
//       </div>
...
//             const isAvailable = bike.status === "Available";
//             return (
//               <div key={bike._id} onClick={() => setSelectedBike(bike)}
//                 className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group cursor-pointer">
//                 <div className="relative h-48 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
//                   {img ? (
//                     <img src={img} alt={bike.bikeName} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
//                   ) : (
//                     <div className="w-full h-full flex items-center justify-center"><FaMotorcycle className="text-5xl text-gray-300" /></div>
//                   )}
//                   <div className="absolute top-3 right-3">
//                     <span className={`px-2 py-0.5 text-xs font-semibold rounded-full ${getStatusStyle(bike.status)}`}>{bike.status}</span>
//                   </div>
//                   {bike.photos?.length > 1 && (
//                     <div className="absolute bottom-2 left-2 bg-black/50 text-white text-xs px-2 py-0.5 rounded-full">{bike.photos.length} photos</div>
//                   )}
//                 </div>
//                 <div className="p-4">
//                   <div className="flex justify-between items-start mb-1">
...
//                   <div className="flex gap-1.5 mb-3 flex-wrap">
//                     <span className="px-2 py-0.5 bg-blue-50 text-blue-600 text-xs font-semibold rounded-full border border-blue-100">{bike.bikeType}</span>
//                     {bike.engineCapacity && <span className="px-2 py-0.5 bg-gray-100 text-gray-500 text-xs rounded-full">{bike.engineCapacity}</span>}
//                     {bike.fuelType && bike.fuelType !== "Petrol" && <span className="px-2 py-0.5 bg-green-100 text-green-600 text-xs rounded-full">{bike.fuelType}</span>}
//                   </div>
//                   <div className="flex items-center justify-between">
//                     <div>
//                       <p className="text-xs text-gray-400">From</p>
//                       <p className="text-xl font-bold text-gray-900">&#2352;{bike.ratePerDay}<span className="text-xs font-normal text-gray-400">/day</span></p>
//                     </div>
//                     <button onClick={(e) => { e.stopPropagation(); if (isAvailable) handleBook(bike); }} disabled={!isAvailable}
//                       className={`flex items-center gap-1.5 px-3.5 py-2 rounded-xl text-sm font-semibold transition-all ${
//                         isAvailable
//                           ? "bg-gradient-to-r from-blue-600 to-blue-700 text-white hover:shadow-lg hover:shadow-blue-500/25 hover:from-blue-700 hover:to-blue-800"
//                           : "bg-gray-100 text-gray-400 cursor-not-allowed"
//                       }`}>
//                       {isAvailable ? <><span>Book</span><FaArrowRight size={11} /></> : "Unavailable"}
//                     </button>
//                   </div>
//                 </div>
//               </div>
...
//       {!loading && filtered.length > 0 && (
//         <p className="text-center text-gray-400 text-sm mt-8">
//           Showing {filtered.length} bike{filtered.length !== 1 ? "s" : ""}{typeFilter !== "All" && ` in ${typeFilter}`}
//         </p>
//       )}
//     </div>
//   );
...
//   return (
//     <>
//       {embeddedMode ? inner : <section id="bikes-section" className="py-16 bg-gradient-to-b from-gray-50 to-white">{inner}</section>}
//       {selectedBike && <BikeDetailModal bike={selectedBike} onClose={() => setSelectedBike(null)} onBook={handleBook} />}
//     </>
//   );
// };
          ...
          <div className="sticky top-0 bg-white border-b border-gray-100 px-8 py-5 flex justify-between items-center z-10">
            <div>
              ...
              <div className="flex items-center gap-2 mt-2 flex-wrap">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusStyle(bike.status)}`}>
                  {bike.status}
                </span>
                {bike.bikeNumber && <span className="text-gray-400 text-xs">#{bike.bikeNumber}</span>}
              </div>
              ...
              {isOnHold && bike.holdExpiresAt && (
                <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-amber-800 text-sm font-semibold">This bike is currently on hold</p>
                    <p className="text-amber-700 text-xs">
                      <strong>{getHoldTimeRemaining(bike.holdExpiresAt)}</strong> — will be released automatically if payment is not completed.
                    </p>
                  </div>
                </div>
              )}
          ...
          <div className="p-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
              {photos.length > 0 ? photos.map((photo, index) => (
                <div key={index} className={index === 0 ? "lg:col-span-2" : ""}>
                  <div className="relative h-64 lg:h-80 rounded-2xl overflow-hidden">
                    <img src={`http://localhost:5000/uploads/bikes/${photo.filename}`} alt={photo.label}
                      className="w-full h-full object-cover" />
                    <div className="absolute bottom-4 left-4 bg-black/60 text-white px-3 py-1.5 rounded-full text-sm backdrop-blur-sm">
                      {photo.label}
                    </div>
              ...
              <div className="lg:col-span-2">
                ...
                {bike.features?.length > 0 && (
                  <div className="mt-10">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {bike.features.map((f, i) => (
                        <span key={i} className="px-4 py-2 bg-blue-50 text-blue-700 rounded-lg font-medium">{f}</span>
                      ))}
                    </div>
                  </div>
                )}
              ...
              <div className="lg:col-span-1">
                <div className="sticky top-24 bg-gradient-to-b from-white to-gray-50 border border-gray-200 rounded-2xl p-6 shadow-lg">
                  <div className="text-center mb-6">
                    ...
                    {bike.ratePerWeek && (
                      <p className="text-sm text-blue-600 mt-2">Weekly: &#2352;{bike.ratePerWeek}</p>
                    )}
                  </div>
                  ...
                  <div className="mb-6 p-4 bg-blue-50 border border-blue-100 rounded-lg">
                    <div className="flex items-start">
                      ...
                      <p className="text-sm text-blue-700">
                        Helmet {bike.helmetIncluded ? "included" : "not included"}.
                        {bike.minimumAge ? ` Min. age ${bike.minimumAge} yrs.` : ""}
                        {bike.securityDeposit > 0 ? ` Deposit: &#2352;${bike.securityDeposit}.` : ""}{" "}
...
const BikesSection = ({ isLoggedIn = false, embeddedMode = false, onCountReady = null }) => {
  ...
  const fetchBikes = async () => {
    try {
      ...
      if (res.data.success) {
        ...
        const availableCount = res.data.data.filter((b) => b.status === "Available").length;
  ...
  const inner = (
    <div className={embeddedMode ? "" : "container mx-auto px-4 sm:px-6"}>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-8 max-w-2xl mx-auto">
        <div className="relative flex-1">
          <FaSearch className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-400" />
          <input type="text" placeholder="Search bikes..." value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-11 pr-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white shadow-sm" />
        </div>
      </div>
            ...
            return (
              <div key={bike._id} onClick={() => setSelectedBike(bike)}
                ...
                <div className="p-4">
                  ...
                  <p className="text-xs text-gray-400 mb-1">{bike.brand} {bike.model}</p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={(e) => { e.stopPropagation(); if (bikeIsAvailable) handleBook(bike); }}
                      ...
                      {bikeIsAvailable
                        ? <><span>Book</span><FaArrowRight size={11} /></>
                        : bikeIsOnHold
                        ? "On Hold"
                        : "Unavailable"}
                    </button>
  ...
  return (
    <>
      {embeddedMode
        ? inner
        : <section id="bikes-section" className="py-16 bg-gradient-to-b from-gray-50 to-white">{inner}</section>
      }
      {selectedBike && (
        <BikeDetailModal
          bike={selectedBike}
