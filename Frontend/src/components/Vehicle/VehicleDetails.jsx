...
const VehicleDetails = () => {
  ...
  const [showDocumentModal, setShowDocumentModal] = useState(false);
  const [ownerProfile, setOwnerProfile] = useState(null);
  // NEW: track whether the logged-in user is the owner of this vehicle
  ...
  const fetchVehicleDetails = async () => {
    try {
      ...
      if (response.data.success) {
        ...
        const ownerId =
          typeof vehicleData.user === "object"
            ? vehicleData.user?._id?.toString()
            : vehicleData.user?.toString();
  ...
  const fetchOwnerProfile = async (userId) => {
    try {
      ...
      // userId may be a populated object { _id, name, ... } or a plain string ID
      const id =
      ...
      // Silently fail — owner profile is non-critical
      console.error("Error fetching owner profile:", error);
    }
  };
      ...
      <div className="container mx-auto px-6 py-8 max-w-6xl">
        ...
        >
          <div className="flex flex-wrap justify-between items-start gap-4">
            ...
            <div className="flex items-center gap-3">
              {/* Owner badge shown in header */}
              {isOwner && (
                <span className="bg-blue-100 text-blue-700 text-sm font-semibold px-4 py-2 rounded-full border border-blue-200">
                  Your Vehicle
                </span>
              )}
              <div
          ...
          <div className="space-y-6">
            ...
            >
              <div className="text-center mb-6">
                ...
                {isOwner && (
                  <span className="mt-2 inline-block text-xs font-semibold bg-blue-100 text-blue-700 px-3 py-1 rounded-full">
                    This is you
                  </span>
                )}
              </div>
            ...
            {!isOwner ? (
              ...
              /* Owner notice instead of book button */
              <motion.div
                initial={{ opacity: 0, x: 20 }}
                animate={{ opacity: 1, x: 0 }}
                transition={{ delay: 0.2 }}
              ...
              >
                <div className="flex items-center gap-3 mb-2">
                  <div className="w-9 h-9 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0">
                    <FaLock className="text-blue-500" size={15} />
                  </div>
                  <p className="text-blue-800 font-semibold text-sm">
                    This is your listed vehicle
                  </p>
                </div>
                <p className="text-blue-600 text-xs ml-12">
                  You cannot book a vehicle you own. Share this page with others
                  so they can book it.
                </p>
                <button
                  onClick={() => navigate("/profiledetails")}
                  className="mt-4 w-full py-2.5 rounded-xl text-sm font-semibold bg-blue-600 hover:bg-blue-700 text-white transition"
                >
                  Back to My Profile
                </button>
              </motion.div>
