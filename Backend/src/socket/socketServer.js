...
//               });
//               console.log(`🔔 Toast notification sent to user_${participantId}`);
//             }
//           }
//         }
  ...
  io.on("connection", (socket) => {
    ...
    socket.on("send_message", async (data) => {
      try {
        ...
        // Notify each recipient in their personal room
        // Re-fetch sender's profilePhoto fresh from DB — socket.user is cached
        // at connection time and may have a stale/null profilePhoto
        let freshSenderPhoto = socket.user.profilePhoto || null;
        try {
          const freshSender = await User.findById(socket.userId).select(
            "profilePhoto",
          );
          if (freshSender?.profilePhoto) {
            freshSenderPhoto = freshSender.profilePhoto;
            // Update cached socket user so next messages don't need a DB hit
            socket.user.profilePhoto = freshSender.profilePhoto;
          }
        } catch (_) {}
        ...
        for (const participantId of chat.participants) {
          if (participantId.toString() !== socket.userId) {
            io.to(`user_${participantId}`).emit("new_message_notification", {
              ...
              vehicleName: chat.vehicleName,
              senderId: socket.userId,
              senderPhoto: freshSenderPhoto,
            });
            console.log(
              `🔔 Notification sent to user_${participantId} (photo: ${freshSenderPhoto ? "✓" : "none"})`,
            );
          }
        }
      } catch (error) {
