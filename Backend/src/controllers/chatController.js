...
// // // //     const io = req.app.get("io");
// // // //     if (io) {
// // // //       io.to(`chat_${chatId}`).emit("chat_blocked", {
// // // //         chatId,
// // // //         blockedBy: userId,
// // // //       });
// // // //     }
...
// // //     // Attach muted flag + pre-compute otherParticipant per user
// // //     const chatsWithMeta = chats.map((c) => {
// // //       const obj = c.toObject();
// // //       obj.isMuted =
// // //         c.mutedBy?.map((id) => id.toString()).includes(userId) || false;

// // //       // Pre-compute otherParticipant so frontend doesn't need to filter
// // //       // This ensures profilePhoto is always available in the chat list
// // //       const other = (obj.participants || []).find(
// // //         (p) => p && p._id && p._id.toString() !== userId.toString(),
// // //       );
// // //       obj.otherParticipant = other
// // //         ? {
// // //             _id: other._id,
// // //             name: other.name,
// // //             email: other.email,
...
// // //         obj.unreadCounts = Object.fromEntries(c.unreadCounts);
// // //       }
...
// //     const chatObj = { ...chat.toObject(), messages: filteredMessages };

// //     // Add vehicleOwnerId so frontend shows correct "Vehicle Owner" vs "Customer" labels
// //     if (chat.chatType === "vehicle") {
// //       const ownerMsg = filteredMessages.find((m) => m.senderType === "owner");
// //       if (ownerMsg) {
// //         chatObj.vehicleOwnerId = (
// //           ownerMsg.sender?._id || ownerMsg.sender
// //         )?.toString();
// //       } else {
...
// //           chatObj.vehicleOwnerId = other._id.toString();
// //         }
// //       }
// //     }
...
// //     // Attach muted flag + vehicleOwnerId per user
// //     // vehicleOwnerId tells the frontend WHO the vehicle owner is in this chat
// //     // so it can show "Vehicle Owner" vs "Customer" labels correctly
// //     const chatsWithMeta = chats.map((c) => {
// //       const obj = c.toObject();
// //       obj.isMuted =
// //         c.mutedBy?.map((id) => id.toString()).includes(userId) || false;

// //       // For vehicle chats: find which participant is the owner
// //       // The owner is whichever participant is NOT the current user
// //       // AND whose messages have senderType "owner"
// //       // We store this as vehicleOwnerId so the frontend knows
// //       if (c.chatType === "vehicle") {
// //         // Look at the first "owner" senderType message to identify the owner
// //         const ownerMsg = c.messages?.find((m) => m.senderType === "owner");
// //         if (ownerMsg) {
// //           obj.vehicleOwnerId = ownerMsg.sender?.toString();
// //         } else {
// //           // If no owner message yet, the owner is the participant who is NOT userId
// //           const otherParticipant = c.participants.find(
// //             (p) => p._id.toString() !== userId.toString(),
// //           );
// //           // Only mark as owner if the chat was created for a user vehicle
// //           // (admin vehicles the "owner" is admin who has role=admin)
// //           if (otherParticipant && otherParticipant.role !== "admin") {
// //             obj.vehicleOwnerId = otherParticipant._id.toString();
// //           }
// //         }
// //       }
...
//     // Attach muted flag + explicitly serialize unreadCounts Map → plain object
//     const chatsWithMeta = chats.map((c) => {
//       const obj = c.toObject();
//       obj.isMuted =
//         c.mutedBy?.map((id) => id.toString()).includes(userId) || false;

//       // Mongoose Map serializes inconsistently — force it to a plain object
//       // so frontend can reliably read unreadCounts[userId]
//       if (c.unreadCounts instanceof Map) {
//         obj.unreadCounts = Object.fromEntries(c.unreadCounts);
//       } else if (obj.unreadCounts && typeof obj.unreadCounts === "object") {
//         // Already plain object from toObject() — ensure string keys
//         const counts = {};
//         for (const [k, v] of Object.entries(obj.unreadCounts)) {
//           counts[k.toString()] = v || 0;
//         }
//         obj.unreadCounts = counts;
//       } else {
//         obj.unreadCounts = {};
//       }
  ...
  try {
    ...
    if (!chat) {
      chat = new Chat({
        ...
        vehicleModel: vehicleType === "user" ? "UserVehicle" : "Vehicle",
        vehicleName,
        vehicleOwnerId: ownerId.toString(),
        title: `Chat about ${vehicleName}`,
        messages: [],
        unreadCounts: new Map([
          [userId.toString(), 0],
      ...
      // Backfill vehicleOwnerId for old chats that don't have it
      if (!chat.vehicleOwnerId && ownerId) {
        chat.vehicleOwnerId = ownerId;
        needsSave = true;
      }
      if (needsSave) await chat.save();
    }
...
export const getChatById = async (req, res) => {
  try {
    ...
    // Force unreadCounts Map → plain object
    if (chat.unreadCounts instanceof Map) {
      chatObj.unreadCounts = Object.fromEntries(chat.unreadCounts);
    }

    // vehicleOwnerId: use stored field, else look up from vehicle doc (old chats)
    if (chat.chatType === "vehicle") {
      ...
      if (!chatObj.vehicleOwnerId) {
        const adminP = chat.participants.find((p) => p.role === "admin");
        if (adminP) chatObj.vehicleOwnerId = adminP._id.toString();
      }
    }
...
export const getUserChats = async (req, res) => {
  try {
    ...
    // Build chat metadata for each chat
    const chatsWithMeta = chats.map((c) => {
      const obj = c.toObject();
      ...
      // Force unreadCounts Map → plain { userId: count } object
      if (c.unreadCounts instanceof Map) {
        obj.unreadCounts = Object.fromEntries(c.unreadCounts);
      } else if (obj.unreadCounts && typeof obj.unreadCounts === "object") {
        const counts = {};
      ...
      // vehicleOwnerId — use stored field if available, else resolve from DB map
      if (c.chatType === "vehicle") {
        if (c.vehicleOwnerId) {
          // Already stored on the chat document (new chats)
          obj.vehicleOwnerId = c.vehicleOwnerId.toString();
        } else if (c.vehicleId && vehicleOwnerMap[c.vehicleId.toString()]) {
          // Fallback: resolved from UserVehicle DB lookup (old chats)
          obj.vehicleOwnerId = vehicleOwnerMap[c.vehicleId.toString()];
        } else {
          // Admin-listed vehicle fallback: admin is the owner
          const adminP = c.participants.find((p) => p.role === "admin");
          if (adminP) obj.vehicleOwnerId = adminP._id.toString();
        }
      }
