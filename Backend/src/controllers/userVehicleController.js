  ...
  try {
    ...
    if (req.files && req.files.citizenshipFront) {
      citizenshipFront = {
        ...
        url: `/uploads/user-vehicles/${req.files.citizenshipFront[0].filename}`,
        uploadedAt: new Date(),
      };
    }
    if (req.files && req.files.citizenshipBack) {
      citizenshipBack = {
        ...
        url: `/uploads/user-vehicles/${req.files.citizenshipBack[0].filename}`,
        uploadedAt: new Date(),
      };
    }
    if (req.files && req.files.passportPhoto) {
      passportPhoto = {
        ...
        originalName: req.files.passportPhoto[0].originalname,
        path: req.files.passportPhoto[0].path,
        url: `/uploads/user-vehicles/${req.files.passportPhoto[0].filename}`,
        uploadedAt: new Date(),
      };
    }
    ...
    res.status(201).json({
      ...
      data: { vehicleId: userVehicle._id, status: userVehicle.status },
    });
  } catch (error) {
...
export const updateUserVehicle = async (req, res) => {
  try {
    ...
    await createNotification(
      userId,
      "Vehicle Updated ✏️",
      `Your vehicle ${vehicle.carName} has been updated.`,
      "info",
...
export const deleteUserVehicle = async (req, res) => {
  try {
    ...
    await createNotification(
      userId,
      "Vehicle Deleted 🗑️",
      `Your vehicle ${vehicle.carName} has been deleted.`,
      "error",
      `Car Number: ${vehicle.carNumber}`,
      { vehicleId: vehicle._id },
    );
...
export const getAllUserVehicles = async (req, res) => {
  try {
    ...
    vehicles.forEach((v) => {
      console.log(
        `- ${v.carName}: status = ${v.status}, isListed = ${v.isListed}`,
      );
    });
...
// Public endpoint for home page (excludes owner's own vehicles)
    ...
    if (currentUserId) {
      ...
      vehicles = vehicles.filter((vehicle) => {
        const vehicleOwnerId =
          vehicle.user?._id?.toString() || vehicle.user?.toString();
        return vehicleOwnerId !== currentUserId;
      });
      console.log(
        `📊 Public vehicles: Showing ${vehicles.length} out of ${originalCount} (excluded own vehicles)`,
      );
    }
...
export const approveUserVehicle = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to approve vehicle" });
  }
};
...
export const rejectUserVehicle = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to reject vehicle" });
  }
};
...
export const activateUserVehicle = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to activate vehicle" });
  }
};
...
export const deactivateUserVehicle = async (req, res) => {
  try {
    ...
    await createNotification(
      ...
      `Your vehicle ${vehicle.carName} (${vehicle.carNumber}) has been deactivated.`,
      "warning",
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to deactivate vehicle" });
  }
};
...
export const deleteUserVehicleAdmin = async (req, res) => {
  try {
    ...
    await createNotification(
      vehicle.user._id,
      "Vehicle Deleted 🗑️",
      `Your vehicle ${vehicle.carName} (${vehicle.carNumber}) has been permanently deleted.`,
      "error",
    ...
    res
      .status(200)
      .json({ success: true, message: "Vehicle deleted permanently" });
  } catch (error) {
...
// ✅ FIXED: getUserEarnings now includes all booking fields
export const getUserEarnings = async (req, res) => {
  try {
    ...
    for (const vehicle of userVehicles) {
      ...
      vehiclesEarnings.push({
        ...
        bookings: bookings.map((b) => ({
          ...
          driverOption: b.driverOption,
          insuranceOption: b.insuranceOption,
          user: b.user,
          createdAt: b.createdAt,
          confirmedAt: b.confirmedAt,
          ownerEarnings: Math.round((b.totalAmount || 0) * 0.7),
        })),
      });
      ...
      bookings.forEach((booking) => {
        allTransactions.push({
          ...
          ownerEarnings: Math.round((booking.totalAmount || 0) * 0.7),
          vehicle: {
            carName: vehicle.carName,
            carNumber: vehicle.carNumber,
            vehiclePhotos: vehicle.vehiclePhotos || [],
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to fetch earnings data" });
  }
};
...
export const getAvailableVehicleForBooking = async (vehicleId) => {
  try {
    return await UserVehicle.findOne({
      _id: vehicleId,
      status: "active",
      isListed: true,
    });
...
export const fixVehicleStatus = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      data: { status: vehicle.status, isListed: vehicle.isListed },
    });
  } catch (error) {
    console.error("Fix vehicle status error:", error);
    res
      .status(500)
      .json({ success: false, message: "Failed to fix vehicle status" });
  }
};
